...
totalcatch <- read_csv("edi.233.3/integrated_wq_totalcatch.csv")
...
totalcatch %>%
  ...
  filter(Count>0) %>%
   ggplot(aes(x = SampleDate, y = Count, color=OrganismCode, shape=OrganismCode)) +
...
#by year
totalcatch$Year <- as.integer(format(totalcatch$SampleDate, format = "%Y"))

totalcatch.yr <- totalcatch %>% 
...
```{r, message=FALSE, warning=FALSE}
fish_unique <- read_csv("edi.233.3/fish_unique.csv")
event <- read_csv("edi.233.3/event.csv")
